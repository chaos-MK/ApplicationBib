...
| cert-manager | cert-manager (Terraform-installed) | Issues and automatically rotates the TLS certificate/CA used by the Vault Agent Injector's mutating admission webhook | Vault Injector Certificate/CA, `vault-agent-injector-tls` Secret | Secrets boundary / Kubernetes control plane | Controls the webhook TLS trust used for admission into workload pods; misconfiguration could break or weaken injector webhook TLS |
...
| 11a | Terraform | cert-manager | Installation/configuration | Install and configure cert-manager | IaC boundary → Secrets boundary |
| 11b | cert-manager | Vault Injector Certificate / CA | Certificate issuance request | Issue the Vault Agent Injector webhook TLS certificate (24h duration, renews 2h before expiry, `rotationPolicy: Always`) | Secrets boundary internal |
| 11c | Vault Injector Certificate / CA | `vault-agent-injector-tls` Secret | Issued TLS certificate + key | Store the webhook TLS certificate for consumption by the Vault Helm release | Secrets boundary internal |
| 11d | `vault-agent-injector-tls` Secret | Vault Agent Injector | TLS certificate + key | Provide webhook TLS material to the injector; cert-manager CA injection registers the CA bundle with the Kubernetes Mutating Webhook Configuration | Secrets boundary → Kubernetes control plane |
...
| cert-manager | Spoofed/unauthorized Certificate or Issuer resource impersonating the Vault Injector Certificate | Unauthorized modification of the Certificate resource, Issuer/CA, or the `vault-agent-injector-tls` Secret | Limited visibility into certificate issuance/rotation events without centralized audit logging | Compromise of the Issuer/CA private key material could allow forged webhook TLS certificates | cert-manager or renewal failure could leave the injector webhook without a valid certificate, blocking admission and pod startup | Overly broad cert-manager RBAC/ClusterRole could allow issuance of certificates beyond the Vault Injector Certificate's intended scope |
...
| Manually renewed / expiring Vault Agent Injector webhook certificate | Terraform-installed cert-manager automatically issues and rotates the Vault Agent Injector webhook TLS certificate (`vault-agent-injector-tls` Secret, 24h duration, renews 2h before expiry, `rotationPolicy: Always`), with CA injection registering the CA bundle to the Kubernetes Mutating Webhook Configuration | Implemented | Compromise of the cert-manager Issuer/CA or the `vault-agent-injector-tls` Secret could still undermine webhook TLS trust | Continue least-privilege RBAC for cert-manager and periodically review Issuer/CA scope |
...
- cert-manager, installed and managed through Terraform, automatically issues and rotates the Vault Agent Injector webhook TLS certificate (stored in `vault-agent-injector-tls`, 24-hour duration, renewed 2 hours before expiry, `rotationPolicy: Always`), removing the previous dependency on manually renewed/expiring injector certificates.
...
10. **Terraform:** Terraform manages the Kubernetes/Helm infrastructure and monitoring configuration, including ingress, kube-prometheus-stack, Loki, Grafana dashboards, ServiceMonitors, exporters, and related monitoring resources. Terraform also installs and manages cert-manager, which in turn issues and rotates the Vault Agent Injector's webhook TLS certificate; the Vault Helm release itself remains outside Terraform's management. Terraform is an infrastructure-as-code control-plane tool and is not a runtime dependency of the frontend or backend.
...
13. **Vault security:** HashiCorp Vault is used as the central secret-management system for sensitive runtime credentials. The Vault Agent Injector injects secrets into the workloads that require them, including the backend's Firebase service-account and PostgreSQL credentials and Alertmanager's SMTP credentials. Vault Kubernetes authentication, workload-scoped policy controls, and secret TTL controls are implemented. The TLS certificate used by the Vault Agent Injector's mutating admission webhook is managed by cert-manager: it is stored in the `vault-agent-injector-tls` Secret, automatically renewed (24-hour duration, renewal 2 hours before expiry, `rotationPolicy: Always`), and registered with the Kubernetes Mutating Webhook Configuration through cert-manager CA injection — removing the previous dependency on manually renewed/expiring injector certificates. Further policy refinement and centralized Vault audit logging remain possible improvements.
...
17. **Implemented security controls:** Firebase ID-token authentication and server-side verification, Vault-managed and runtime-injected Firebase/PostgreSQL/SMTP credentials, Kubernetes-authenticated Vault access, Vault TTL controls, cert-manager–automated Vault Agent Injector webhook TLS issuance/rotation, Kubernetes NetworkPolicies, reviewed/tightened RBAC, secrets/SAST/dependency/container scanning, SBOM generation, Dockerfile linting, Kubernetes manifest/security validation, OWASP ZAP DAST, monitoring/alerting, and Cloudflare Quick Tunnel-based external access are implemented controls.
