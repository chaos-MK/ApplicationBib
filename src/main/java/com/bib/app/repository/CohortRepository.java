...
public interface CohortRepository extends JpaRepository<Cohort, Long> {
    ...
    // Split the problematic query into separate queries to avoid MultipleBagFetchException
    @Query("SELECT DISTINCT c FROM Cohort c " +
           "LEFT JOIN FETCH c.users " +
           "WHERE c.project.projectId = :projectId")
    List<Cohort> findByProjectIdWithUsers(@Param("projectId") Long projectId);
    
    @Query("SELECT DISTINCT c FROM Cohort c " +
           "LEFT JOIN FETCH c.stages " +
           "WHERE c.project.projectId = :projectId")
    List<Cohort> findByProjectIdWithStages(@Param("projectId") Long projectId);
    
    // Alternative: Use a basic query and let the service handle lazy loading
    @Query("SELECT c FROM Cohort c WHERE c.project.projectId = :projectId")
    List<Cohort> findByProjectIdBasic(@Param("projectId") Long projectId);
}